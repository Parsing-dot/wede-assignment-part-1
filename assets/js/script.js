// ================= PIXEL CANVAS ANIMATION (Physics-Based Staggered Ripple) =================
class PixelCanvasAnimation {
    constructor(canvasId) {
        ...
        this.mouseX = -9999;
        this.mouseY = -9999;
        this.rippleWave = null; // { x, y, startTime, radius }

        // Mdukazi palette — subtle on gradient bg
        this.colors = [
        ...
        hero.addEventListener('pointermove', (e) => {
            ...
            // Drive the radial vignette via CSS custom properties
    ...
    triggerRipple(x, y) {
        ...
        // Re-trigger pixels outward from click origin
    ...
    createPixel(x, y, delay) {
        ...
        return {
            ...
            maxSize: r(0.6, 2.2),
            maxSizeInt: 2,
            delay,          // ms — centre-out wave
            elapsed: 0,
            phase: 'appear',   // appear | shimmer | ripple-out
            shimmerDir: 1,
    ...
    init() {
        ...
        for (let x = 0; x < this.w; x += this.gap) {
            for (let y = 0; y < this.h; y += this.gap) {
                ...
                const dist = Math.sqrt(dx * dx + dy * dy);
                // counterStep-style: larger canvas = longer delay spread
    ...
    loop(now) {
        ...
        const dt = Math.min(elapsed, 32); // cap delta

        for (const p of this.pixels) {
            // ---- ripple-out phase (click wave) ----
            ...
            if (p.phase === 'appear') {
                ...
                // Mouse proximity boosts brightness/size
                const mdx = p.x - this.mouseX;
                const mdy = p.y - this.mouseY;
                const mDist = Math.sqrt(mdx * mdx + mdy * mdy);
                const proximity = Math.max(0, 1 - mDist / 90); // influence radius 90px
...
// ================= ORIGIN BUTTON (Next-Level Physics Ripple) =================
class OriginButton {
    constructor(el) {
        this.el = el;
    ...
    _onDown() {
        this.el.classList.add('origin-pressed');
    }
...
function initScrollReveal() {
    ...
    targets.forEach(el => {
        // Only observe elements not already animated by CSS delay
        if (!el.style.animationDelay && !el.style.getPropertyValue('--delay')) {
            observer.observe(el);
        } else {
            observer.observe(el);
        }
...
function animateCounters() {
    // purely visual — only fires if stat contains a real number
