...
function loadSpecials() {
  ...
  if (!active.length) { specialsList.innerHTML = '<p>No active specials at the moment.</p>'; return; }
...
function submitIssue() {
  ...
  cd.issues.push({ id: Date.now(), type: issueType, description, status: 'pending', createdAt: new Date().toISOString() });
...
function processPayment() {
  ...
  if (method === 'card') {
    ...
    if (!cn || !ce || !cv) { alert('Please fill in all card details'); return; }
  }
  const user          = window.auth.getCurrentUser();
  const cd            = user.clientData;
  const currentMonth  = new Date().toISOString().substring(0, 7);

  cd.paymentHistory.push({ id: Date.now(), amount: cd.balance, method, date: new Date().toISOString(), service: cd.service, period: currentMonth });
...
function bookService() {
  ...
  if (!serviceType) { alert('Please select a service'); return; }
